...
main :: IO ()
main = interact $ folderToDelete
    where
        sizes = (\cmds -> (getDirSizes <$> runCreateFS cmds)) . parseList parseCommand
        totalSpace str = head <$> sizes str
        folderToDelete str = minimum . filter (> (30000000 - (70000000 - (fromJust (totalSpace str))))) <$> sizes str
