...
run str = minimum $ map length $ catMaybes finalState
    where 
        elevationMap = tupleMap str
        finalMap = M.adjust (const 'z') (fst end) $ M.adjust (const 'a') (fst start) elevationMap
        finalState =  bfs (getNeighbors finalMap) (\x ->x == (fst end)) <$> possibleStarts
        start = head $ M.toList $ M.filter (== 'S') elevationMap
        possibleStarts = map fst $ M.toList $ M.filter (== 'a') elevationMap
